...
const stringSameness = (valA, valB, options) => {
    const { transforms = index_js_2.strDefaultTransforms, strategies = defaultStrategies, reorder = false, delimiter = ' ' } = options || {};
    ...
    if (reorder) {
        ...
        const [orderedX, orderedY] = (0, exports.reorderStr)(cleanA, cleanB);
        cleanA = orderedX;
        cleanB = orderedY;
    }
...
const reorderStr = (strA, strB, options) => {
    const { transforms = index_js_2.strDefaultTransforms, strategies = defaultStrategies, delimiter = ' ' } = options || {};
